...
class EKF_imu:
    def __init__ (self, del_t, accel0, mag0, frame, magnetic_declin_angle, magnetic_inclin_angle, GN, AN, MN):
        ...
        gyro_dev = np.array([GN**2, GN**2, GN**2]) # 0.1 - 0.3
        # Spectral noise covariance matrix NOT REQUIRED IF GYRO NOISE SAME
        self.sncm = np.diag(gyro_dev)
        self.accel_dev = np.array([AN**2, AN**2, AN**2]) # 3.6 - 14.4 variance for accelerometer noise
        self.mag_dev = np.array([MN**2, MN**2, MN**2]) # 0.3 - 1.4 variance for gyroscope noise
        ...
        '''
        print(f"[INIT] Frame: {self.frame}")
        print(f"[INIT] Gravity reference (g): {self.g}")
        print(f"[INIT] Magnetic reference (r): {self.r}")
        print(f"[INIT] r norm: {np.linalg.norm(self.r)} (should be ~1.0)")
        '''
        # print(self.r)
        ...
        self.q = in_calculate_initial_quaternion(accel0, mag0,
                                      ...
                                      accel_weight=1.5,
                                      mag_weight=1.0)
        # print(f"[INIT] Initial quaternion q0: {self.q}")
    ...
    def correct(self, accel, mag):
        ...
        '''
        print(f"[CORRECT] Raw accel: {accel}, norm: {np.linalg.norm(accel)}")
        print(f"[CORRECT] Raw mag: {mag}, norm: {np.linalg.norm(mag)}")
        print(f"[CORRECT] Accel residual: {normalize(accel) - normalize(self.a_hat)}")
        print(f"[CORRECT] Mag residual: {normalize(mag) - normalize(self.m_hat)}")
        print(f"[CORRECT] v (innovation): {self.v}")
        print(f"[CORRECT] Kalman gain K: {self.K}")
        print(f"[CORRECT] Quaternion after correction: {self.q}")
